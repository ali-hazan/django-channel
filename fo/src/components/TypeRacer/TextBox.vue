<template>
  <div class="text-box">
    ...
    <span class="current-word"
      ...
      }}<span class="current-letter">{{ currentWord[currentLetterIndex] }}</span
      >{{ currentWord.slice(currentLetterIndex + 1) }}</span
    >
...
<script setup>
import { watch,toRefs,ref, computed,nextTick } from 'vue'
...
watch(word, (val) => {
  if (
    val.replace(/ +/g, '') === currentWord.value.replace(/ +/g, '') &&
    (val[val.length - 1] === ' ' || textArray.value.length - 1 === currentWordIndex.value)
  ) {
    onNextWord()
    emit('next-word')
    return
